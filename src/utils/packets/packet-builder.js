...
PacketBuilder.parseRxPacket = function({packet, remote, layoutMap}={}){
  bp = new ButtonPacket()
  bp.remoteAddress = remote.address

  const data_8 = packet.slice(0);
  const data_8v = new Uint8Array(data_8);
  // Map address to display / power unit by IP address
  let displayUnitNumber = layoutMap.displayUnitIPMap[bp.remoteAddress];//DisplayController.displayUnitIPMap[info.remoteAddress];
  let powerUnitNumber = layoutMap.powerUnitIPMap[bp.remoteAddress];//DisplayController.powerUnitIPMap[info.remoteAddress];

  if(bp.remoteAddress == UDPConfig.emulatorIP){
    displayUnitNumber = data_8v[1];
    powerUnitNumber = data_8v[1];
  }

  console.log(displayUnitNumber, powerUnitNumber)
